...
void robarCarta(carta mano[], int contadorMano, int *puntos, int *num_cartas_mano, int *contadorAs);
bool resultado(int *puntos, bool repetir);

int num_cartas_mano, carta_mano_maquina, puntos, contadorAs, contadorMano, puntosMaquina, turno = 0, opcion;
bool repetir = true;  
...
int main(){
  ...
  do {
        ...
        if (turno == 0){
          ...
          puntos = 0;
          contadorAs = 0;
          for (contadorMano = 0; contadorMano < 52; contadorMano++){
            if (mano[contadorMano].palo != ""){
              robarCarta(mano, contadorMano, &puntos, &num_cartas_mano, &contadorAs);
            }
          }
        ...
        switch (opcion) {
            case 1:
                ...
                robarCarta(mano, num_cartas_mano+1, &puntos, &num_cartas_mano, &contadorAs);
                resultado(&puntos, repetir);
                break;
            case 2:               
                cout << "\033[1;31m \nTus cartas son: \033[0m" << endl;
